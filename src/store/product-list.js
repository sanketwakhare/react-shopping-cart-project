...
export const loadProducts = (category) => {
    return (dispatch) => {
        ...
        const categoryWiseProductsApiURL = `https://fakestoreapi.com/products/category/${category}`;
        const url = category === 'home' ? allProductsApiURL : categoryWiseProductsApiURL;
        fetch(url)
            .then((res) => res.json())
            .then((data) => {
